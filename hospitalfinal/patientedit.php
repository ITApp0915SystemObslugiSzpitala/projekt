...
if ($_POST["button"]=="Wybierz"){
    header ('Location: patientinfo.php?id='.$_POST['patient_id'].'');
    }
session_start();
...
<div class="container theme-showcase" role="main">
      ...
      <div class="jumbotron">
		...
		<input type="hidden" name="searchid" id="searchid" value="<?php echo $_POST['patient_id'] ?>" />
		<?php
		...
		$result = $db->query("SELECT patients.* FROM patients where patient_id=".$_POST['patient_id']);
		while($row = $result->fetch_assoc()){
		echo '
		
	<div class="input-group">
  <span class="input-group-addon">ID pacjenta</span>
  <input type="text" id="patient_id" name="patient_id" class="form-control" value="'.$_POST['patient_id'].'" readonly></div>
   <div class="input-group">
...
$result = $db->query("SELECT users.*, staff.* FROM users, staff where staff.user_id=users.user_id ");
...
echo '
 <div class="input-group">
  <span class="input-group-addon">Lekarz Prowadzacy</span>
  <select id="user_id" name="user_id" class="form-control" placeholder="Wybierz Lekarza prowadzacego" autocomplete="off"  data-toggle="popover" onClick=pop()  data-content="Wybierz Lekarza">
  ';
  $db->close();
 while($row = $result->fetch_assoc()){ echo '
  <option value="'.$row{'staff_id'}.'">'.$row{'surname'}.'</option>';}
