...
$result = $db->query("SELECT operation_codes.* FROM operation_codes  ");
...
echo '
 ...
 while($row = $result->fetch_assoc()){ echo '
  <option value="'.$row{'operation_code'}.'">'.$row{'short_description'}.'</option>';}
...
$result = $db->query("SELECT rooms.* FROM rooms  ");
...
echo '
 ...
 ';
 while($row = $result->fetch_assoc()){ echo '
  <option value="'.$row{'room_id'}.'">'.$row{'room_code'}.'</option>';}
...
<td><a href="patientsymptoms.php?patient='.$_GET{'patient'}.'&id='.$_GET{'id'}.'&recordid='.$_GET{'recordid'}.'&diagnoseid='.$row{'diagnose_id'}.'">Więcej...</a></td>
              </tr>';
}
...
$result = $db->query("SELECT diagnose_codes.* FROM diagnose_codes  ");
...
echo '
 ...
 <div class="input-group">
  ...
  while($row = $result->fetch_assoc()){ echo '
  <option value="'.$row{'diagnose_code'}.'">'.$row{'name'}.'</option>';}
